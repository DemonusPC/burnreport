...
export const getSpiSearchSuggestions = async (
  ...
  const request = await fetch(encodeURI(`/api/search?p=${suggestion}&e=spi`));
...
export const postRecipie = generatePostRequest<RecipieCreateCommand, never>(
  "/api/recipies"
);
...
export const postSpi = generatePostRequest<Spi, ProductAPIStatus>("/api/spi");
